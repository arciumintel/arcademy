...
function NavLink({
  ...
  return (
    <Link
      ...
      className={[
        "group/navlink relative block isolate font-ui text-[0.86rem]",
        colorCls,
        collapsed ? "mx-auto h-10 w-10" : "mx-2 h-9 pl-6 pr-3",
      ].join(" ")}
      style={{
    ...
    >
      <span
        ...
        className={[
          ...
          collapsed
            ...
            : "inset-0 rounded-[3px]",
          active ? "opacity-70 scale-100" : "opacity-0 scale-95",
      ...
      <span
        aria-hidden
        className="absolute left-2.5 top-1/2 -translate-y-1/2 font-display italic text-[0.72rem] text-accent"
        style={{
          opacity: collapsed ? 0 : active ? 1 : 0,
      ...
      {!active && !collapsed ? (
        <span
          aria-hidden
          className="pointer-events-none absolute left-2.5 top-1/2 -translate-y-1/2 -translate-x-1 font-display italic text-[0.72rem] text-accent opacity-0 group-hover/navlink:translate-x-0 group-hover/navlink:opacity-100"
          style={{
      ...
      {collapsed ? (
        <span
          aria-hidden
          className="pointer-events-none absolute left-full top-1/2 ml-4 -translate-y-1/2 -translate-x-1 whitespace-nowrap rounded-[2px] border border-ink bg-ink px-2.5 py-1 font-ui text-[0.72rem] text-paper-deep opacity-0 shadow-leaf group-hover/navlink:translate-x-0 group-hover/navlink:opacity-100"
          style={{
...
function SectionLabel({ numeral, children, collapsed }: { numeral: string; children: React.ReactNode; collapsed: boolean }) {
  return (
    <p
      className={[
        "mb-1 flex items-baseline gap-2 font-display italic text-[0.78rem] text-ink-soft",
    ...
    >
      ...
      {!collapsed ? (
        <span aria-hidden className="ml-2 inline-block h-px w-5 self-center bg-rule-strong" />
      ) : null}
    </p>
  );
}
...
export default function PortalNav() {
  ...
  return (
    <nav
      className="fixed bottom-0 left-0 top-0 z-50 hidden flex-col border-r border-ink bg-paper-shade md:flex"
      style={{ width: collapsed ? "56px" : "280px", transition: `width 320ms ${EASE}` }}
      aria-label="Primary"
    >
      <span aria-hidden className="pointer-events-none absolute inset-y-0 right-0 w-px bg-ochre/30" />

      <Link
        href="/"
        aria-label="Arcidex home"
        className="mt-10 mb-12 block"
        style={{
          paddingLeft: collapsed ? "0" : "1.75rem",
          paddingRight: collapsed ? "0" : "1.75rem",
          textAlign: collapsed ? "center" : "left",
          transition: `padding 320ms ${EASE}`,
        }}
      >
        <span
          className="inline-block font-display leading-none text-ink"
          style={{
            fontWeight: 300,
            fontStyle: "italic",
            letterSpacing: "-0.025em",
            fontSize: collapsed ? "1.2rem" : "1.55rem",
      ...
      <div className="flex flex-1 flex-col">
        <div className="mb-7">
          <SectionLabel numeral="i." collapsed={collapsed}>The hub</SectionLabel>
          {HUB.map((entry) => (
        ...
        <div>
          <SectionLabel numeral="ii." collapsed={collapsed}>Account</SectionLabel>
          {ACCOUNT.map((entry) => (
            <NavLink key={entry.href} entry={entry} collapsed={collapsed} />
          ))}
          <NavLink
